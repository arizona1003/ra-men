...
## Web プレビュー版（iPhone / ブラウザで動作確認）

Mac を使わずに iPhone から UI を触れるよう、`docs/` ディレクトリに HTML/CSS/JS 版を同梱しています。iOS 版とほぼ同じデザイン・サンプルデータで、ラー活の記録・行きたいリスト・写真添付まで動作します（データは `localStorage` に保存）。

### GitHub Pages で公開

1. このリポジトリの **Settings → Pages** を開く
2. **Source: Deploy from a branch** を選択
3. **Branch: `main`（または現在のブランチ）/ フォルダ: `/docs`** に設定して Save
4. 数十秒後に `https://<github-username>.github.io/ra-men/` にアクセス
5. iPhone Safari で開き、「共有 → ホーム画面に追加」でスタンドアロン起動
